...
function TodoList() {
    ...
    useEffect(() => {
      ...
      const deleteTodo = async (id)=>{
        ...
        setTodo(newTodo)
        const request = await fetch(`http://localhost:8080/api/todo/${id}`,{method:'DELETE'});
      }
  ...
  return (
    <>
        <h1>Todo List</h1>
        <input id="todo-input" type="text" placeholder="Enter todo title ..." value={input} onChange={(e) => setInput(e.target.value)}/>
        <button id="btn-add" onClick={()=>addNewTodo()}>Add</button>
        <ul>
        {todo.map((item) => (
                  <li>
